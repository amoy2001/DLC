document.addEventListener('DOMContentLoaded', function () {
  const processOptions = [
    "Kuster CPB", "Goller CPB", "Shirting CPB", 
    "Pad Dry", "E-Control"
  ];
  ...
  const stages = {
    ...
    "Single Bath Vat": ["Padding", "Drying", "Heat Set", "Developing", "Drying", "Carbonize", "Drying"],
  ...
  const machineSelectionContainer = document.getElementById('machineSelectionContainer');
  const routeSelectionContainer = document.getElementById('routeSelectionContainer');
  const processListContainer = document.getElementById('processListContainer');
  const developedListContainer = document.getElementById('developedListContainer');
  ...
  processOptions.forEach(machine => {
    ...
    machineDiv.addEventListener('click', () => {
      ...
      document.querySelectorAll('#machineSelectionContainer div').forEach(div => div.classList.remove('selected-machine'));
  ...
  const updateRouteOptions = () => {
    ...
    const routeOptions = selectedMachine && ["Kuster CPB", "Goller CPB", "Shirting CPB"].includes(selectedMachine) 
      ? ["CPB", "Perma White"]
    ...
    routeOptions.forEach(route => {
      ...
      routeDiv.addEventListener('click', () => {
        selectedRoute = route;
        document.querySelectorAll('#routeSelectionContainer div').forEach(div => div.classList.remove('selected-machine'));
  ...
  const renderProcessList = () => {
    ...
    processList.forEach((process, index) => {
      ...
      stageClock.innerText = `Current Stage: ${process.stages[process.stageIndex]} - Time: ${formatTime(process.stageTime)}`;
